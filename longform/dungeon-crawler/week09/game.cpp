...
void debugSummary(const Game& game) {
#ifdef DEBUG
    int itemCount = 0;
    Item* current = game.inventory;
    while (current) {
    ...
    cerr << "[debug] map " << game.rows << "x" << game.cols
         << " player (" << game.player.row << "," << game.player.col << ")"
         << " hp " << game.player.hp
         << " monsters " << game.monsterCount
         << " items " << itemCount << endl;
#else
    (void)game;
#endif
}
